using System;
using System.Data;
using System.Data.SqlClient;
using ClassLibraryMVP;
using ClassLibraryMVP.DataAccess;
using System.Text;
using Raj.EC;
using System.Web.UI.WebControls;

public partial class Reports_Booking_Frm_Booking_Dest_Wise : System.Web.UI.Page
{
    #region Declaration
    private DataSet ds;
    decimal ChargedWeight, Actual_Weight, Articles, Freight, Lorry_Hire_Charge,No_of_CN,All_OtherCharges,ServiceTaxAmount,Total_Freight,Local_Charges;
    int _Menu_Item_Id;
    #endregion

    #region ControlsValues
 
    public int Menu_Item_Id
    {
        set { _Menu_Item_Id = value; }
        get { return _Menu_Item_Id; }
    }

    #endregion

    protected void Page_Load(object sender, EventArgs e)
    {
       Menu_Item_Id = Raj.EC.Common.GetMenuItemId();
      
        if (IsPostBack == false)
        {
            CommonSetting(); 
        }
 

    }
    
    private void CommonSetting()
    {
        if (Menu_Item_Id == 5227)
        {
            this.Title = "MonthWise Booking Register";
            lbl_Heading.Text = "MonthWise Booking Register";
            Wuc_GC_Parameters1.Set_ddl_dly_type_visibility = false;  
        } 
    }

    protected void btn_view_Click(object sender, EventArgs e)
    {
        DateCommon objDateCommon = new DateCommon();
        string msg = "";

         
            lbl_Error.Text = "";
            int Region_Id = Wuc_Region_Area_Branch1.RegionID;
            string Region_Name = Wuc_Region_Area_Branch1.SelectedRegionText;

            int Area_id = Wuc_Region_Area_Branch1.AreaID;
            string Area_Name = Wuc_Region_Area_Branch1.SelectedAreaText;

            int Branch_id = Wuc_Region_Area_Branch1.BranchID;
            string Branch_Name = Wuc_Region_Area_Branch1.SelectedBranchText;

            int BookingTypeId = Wuc_GC_Parameters1.Booking_Type_ID;
            string BookingTypeName = Wuc_GC_Parameters1.Booking_Type;

            int PaymentTypeId = Wuc_GC_Parameters1.Payment_Type_ID;
            string PaymentTypeName = Wuc_GC_Parameters1.Payment_Type; 

            
              
            bool Is_BookingRpt = Convert.ToBoolean(rbtn_Is_BookingRpt.SelectedValue);    

            StringBuilder Path = new StringBuilder(Util.GetBaseURL());
            Path.Append("/");
             
                Path.Append("Reports/Booking/FrmBooking_ReportsViewer.aspx?Menu_Item_Id=" + Util.EncryptInteger(Raj.EC.Common.GetMenuItemId())
                     + "&Region_Id=" + Util.EncryptInteger(Region_Id) + "&Region_Name=" + Util.EncryptString(Region_Name)
                     + "&Area_id=" + Util.EncryptInteger(Area_id) + "&Area_Name=" + Util.EncryptString(Area_Name)
                     + "&Branch_id=" + Util.EncryptInteger(Branch_id) + "&Branch_Name=" + Util.EncryptString(Branch_Name)
                     + "&BookingTypeId=" + Util.EncryptInteger(BookingTypeId) + "&BookingTypeName=" + Util.EncryptString(BookingTypeName)
                     + "&PaymentTypeId=" + Util.EncryptInteger(PaymentTypeId) + "&PaymentTypeName=" + Util.EncryptString(PaymentTypeName)
                     + "&Is_BookingRpt=" + Util.EncryptBool(Is_BookingRpt)); 
             

            StringBuilder sb = new StringBuilder();
            sb.Append("<script type = 'text/javascript'>");
            sb.Append("Open_Details_Window('");
            sb.Append(Path);
            sb.Append("');");
            sb.Append("</script>");

            ClientScript.RegisterStartupScript(this.GetType(), "script", sb.ToString());
 
    } 

    
    public void ClearVariables()
    {
        ds = null;
    }
    protected void btn_null_session_Click(object sender, EventArgs e)
    {
        ClearVariables();
        Response.Write("<script language='javascript'>{self.close()}</script>");
    }
  
}
